...
class Selected extends \Magento\Backend\App\AbstractAction
{
	...
	public function execute()
	{
		...
		if ($arrayKey && $id && $attribute && $conditionName && $valueName) {
			...
			$rule = $this->_objectManager->create('\Dotdigitalgroup\Email\Model\Rules')->load($id);
			//rule not found
			if (! $rule->getId()) {
				$this->_http->getHeaders()->clearHeaders();
				return $this->_http->setHeader('Content-Type', 'application/json')->setBody('Rule not found!');
			}
			...
			$type = $this->_objectManager->create('\Dotdigitalgroup\Email\Model\Adminhtml\Source\Rules\Type')
