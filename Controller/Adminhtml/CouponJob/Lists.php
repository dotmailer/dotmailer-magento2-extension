...
class Lists extends Action
{
    ...
    /**
     * Fetch address-book lists for the coupon job audience select.
     *
    ...
    public function execute(): \Magento\Framework\Controller\Result\Json
    {
        ...
        try {
            $lists         = $this->getRecords($websiteId);
            $scoredResults = $this->filterAndRankRecords($lists, $query);
    ...
    /**
     * Return all address book lists for the given website, loading from cache when available.
     *
     * @param int $websiteId
     * @return array
    ...
    private function getRecords(int $websiteId): array
    {
        ...
        $lists = $this->dotdigitalClient->create(['data' => ['websiteId' => $websiteId]])
            ->addressBooks
            ->show()
            ->getList();
