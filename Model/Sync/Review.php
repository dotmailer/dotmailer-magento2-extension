...
class Review
{
	...
	private function _exportReviewsForWebsite( $website)
	{
		...
		if($reviews->getSize()){
			foreach($reviews as $review){
				try {
					$mageReview = $this->_objectManager->create('Magneto\Review\Model\Review')->load($review->getReviewId());
					$product = $this->_objectManager->create('Magento\Catalog\Model\Product')
					               ->setStoreId($mageReview->getStoreId())
					               ->load($mageReview->getEntityPkValue());

					$customer = $this->_objectManager->create('Magento\Customer\Model\Customer')->load($mageReview->getCustomerId());

					$connectorReview = $this->_objectManager->create('Dotdigitalgroup\Email\Model\Customer\Review')
					...
					foreach($votesCollection as $ratingItem){
						$rating = $this->_objectManager->create('Dotdigitalgroup\Email\Model\Customer\Review\Rating')
	...
	{
		return $this->_objectManager->create('Dotdigitalgroup\Email\Model\Review')->getCollection()
	...
	private function _setImported($ids)
	{
		try{
			...
			$ids = implode(', ', $ids);
			$now = new \Datetime();
			$nowDate = $this->dateTime->format($now->getTimestamp());
