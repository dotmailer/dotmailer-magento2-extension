	...
	//extends \Dotdigitalgroup\Email\Model\Rest
{
    ...
    //rest api data
    const REST_ACCOUNT_INFO                     = 'account-info';
    const REST_CONTACTS                         = 'contacts/';
    const REST_CONTACTS_IMPORT                  = 'contacts/import/';
    const REST_ADDRESS_BOOKS                    = 'address-books/';
    const REST_DATA_FILEDS                      = 'data-fields';
    const REST_TRANSACTIONAL_DATA_IMPORT        = 'contacts/transactional-data/import/';
    const REST_TRANSACTIONAL_DATA               = 'contacts/transactional-data/';
    const REST_CAMPAIGN_SEND                    = 'campaigns/send';
    const REST_CONTACTS_SUPPRESSED_SINCE        = 'contacts/suppressed-since/';
    const REST_DATA_FIELDS_CAMPAIGNS            = 'campaigns';
    const REST_SMS_MESSAGE_SEND_TO              = 'sms-messages/send-to/';
    const REST_CONTACTS_RESUBSCRIBE             = 'contacts/resubscribe';
    const REST_CAMPAIGN_FROM_ADDRESS_LIST       = 'custom-from-addresses';
    const REST_CREATE_CAMPAIGN                  = 'campaigns';
    const REST_PROGRAM                          = 'programs/';
    const REST_PROGRAM_ENROLMENTS               = 'programs/enrolments';
    const REST_TEMPLATES                        = 'templates';
...
//		);
	...
	/**
	 * Validate using the account info request.
	 *
	 * @param $apiUsername
	 * @param $apiPassword
	 *
    ...
    {
        $request = $this->client->get(self::REST_ACCOUNT_INFO, ['auth' => [$apiUsername, $apiPassword]]);
    ...
    public function postAddressBooks($name, $visibility = 'Public')
    {
	    ...
	    $url = self::REST_ADDRESS_BOOKS;
	    $response = $this->executePost($url, $data);
...
//        }
    ...
    {
	    $request = $this->client->get(self::REST_DATA_FILEDS);
    ...
    public function getAccountInfo()
    {
	    ...
	    $response = $this->client->get(self::REST_ACCOUNT_INFO);
...
//$this->_logger->info($message);
	...
	public function executePost( $url, $data )
	{
		$response = $this->client->post($url, $data);
		$message  = 'API post ; ' . $url;
		//$this->_logger->info($message);
