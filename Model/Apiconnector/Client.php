...
class Client extends \Dotdigitalgroup\Email\Model\Apiconnector\Rest
{
	...
	/**
	 * @param int $username apiusernname
	 * @param \Dotdigitalgroup\Email\Helper\Data $password apipassword
    ...
    public function deleteContactsTransactionalData($key, $collectionName = 'Orders')
    {
        ...
        if (isset($response->message))
            $this->_helper->log('DELETE CONTACTS TRANSACTIONAL DATA : ' . $url . ' ' . $response->message);
	...
	{
		$recommended = \Dotdigitalgroup\Email\Helper\Config::API_ENDPOINT . '/recommendations/email';
		$token = $this->_helper->getWebsiteConfig(\Dotdigitalgroup\Email\Helper\Config::XML_PATH_CONNECTOR_DYNAMIC_CONTENT_NOSTO);
