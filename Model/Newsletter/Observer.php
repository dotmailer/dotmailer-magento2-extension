...
class Observer
{
		...
		try {
			...
			if ($subscriber->isSubscribed()) {
				...
				if ($client) {
					...
					$contactEmail->setSubscriberStatus(
				...
				if ($client) {
					...
					$addressBookId = $this->_helper->getSubscriberAddressBook(
						$websiteId
					);
	...
	protected function _addSubscriberToAutomation($email, $subscriber,
		$websiteId
	) {
		...
		try {
			...
			$enrolment = $this->_objectManager->create(
